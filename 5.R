...
eq_ZCFO
eq_ZUFO <- paste("UFO", " - ", P[1] + P[5], " + ", P[2], " * Income +", P[3], " * Population")
eq_ZUFO
...
# Эксперимент 1. Фиктивная переменная для Дальневосточного федерального округа
# вместо фиктивной переменной для центрального федерального округа
...
ZSFO[M$FO=="SFO"]<-1
ZDVFO[M$FO=="DVFO"]<-1
M1<-data.frame(M,ZDVFO,ZUFO,ZSZFO,ZUrFO,ZPFO,ZSKFO,ZSFO)
rm(ZCFO,ZUFO,ZSZFO,ZUrFO,ZPFO,ZSKFO,ZSFO, ZDVFO)

M1

reg4.1<-lm(M1$MilkPrice ~ M1$Income +M1$Population + M1$ZDVFO + M1$ZUFO + M1$ZSZFO + M1$ZUrFO + M1$ZPFO + M1$ZSKFO + M1$ZSFO)
summary(reg4.1) # Коэффициенты сильно изменились
accuracy(reg4.1)
P <- coef(reg4.1)
P
# Фактор Федеральный округ оказывает влияние на Цены на молоко, тк все коэффициенты значимы
# Набольшая цена на молоко в дальневосточном Ф. округе
# Наименьшая цена в приволжском Ф. округе
# Построенную модель можно использовать для прогнозирования

eq_ZCFO <- paste("CFO", " - ", P[1], " + ", P[2], " * Income +", P[3], " * Population")
eq_ZCFO
eq_ZUFO <- paste("UFO", " - ", P[1] + P[5], " + ", P[2], " * Income +", P[3], " * Population")
eq_ZUFO
eq_ZSZFO <- paste("SZFO", " - ", P[1] + P[6], " + ", P[2], " * Income +", P[3], " * Population")
...
eq_ZSFO <- paste("SFO", " - ", P[1] + P[10], " + ", P[2], " * Income +", P[3], " * Population")
eq_ZSFO
eq_ZDVFO <- paste("DVFO", " - ", P[1] + P[4], " + ", P[2], " * Income +", P[3], " * Population")
eq_ZDVFO
...
points(x=M$Income[M$FO=="CFO"], y=M$MilkPrice[M$FO=="CFO"], col=1)
abline(a=coef(reg4.1)[3] * mean(M$Population[M$FO=="CFO"]) + coef(reg4.1)[1] , b=coef(reg4.1)[2], col=1)
...
points(x=M$Income[M$FO=="DVFO"], y=M$MilkPrice[M$FO=="DVFO"], col=8)
abline(a=coef(reg4.1)[3]*mean(M$Population[M$FO=="DVFO"])+coef(reg4.1)[1] + coef(reg4.1)[4], b=coef(reg4.1)[2], col=8)

legend(10000,83,legend = c("ЦФО","ЮФО","СЗФО","УФО","ПФО","СКФО","СФО","ДВФО"), col = c(1,2,3,4,5,6,7,8), lwd=1, pch = c(1, 1, 1,1,1,1,1,1))


# Эксперимент 2. Фиктивная переменная для Дальневосточного федерального округа
# вместо фиктивной переменной для южного федерального округа
...
# Фактор Федеральный округ влияет на коэффициент при факторе Среднедушевые доходы
...
# Фактор Федеральный округ влияет на коэффициент при факторе Численность населения.
...
# Графики
plot(x=M$Population, y=M$MilkPrice, main="Зависимость цены от численности населения" ,xlab = "Численность населения", ylab = "Цена на молоко")
